...
const ohMy = () => {
    ...
    .then(function (res) {
        for  (let i = 0; i < res.data.length; i++){
           ...
           body.appendChild(p.textContent)
        }
      })
      .catch(error => {
        console.log(error)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/moose')
    .then((res) => {
...
const getRequest = () => {
    axios.get('http://localhost:3000/query-test/?number=33')
    .then((res) => {
        console.log(res.data)
    })
}
